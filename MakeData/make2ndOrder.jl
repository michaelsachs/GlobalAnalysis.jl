...
gaussian_func(wave) = 100 * exp(-((wave - 630)^2) / (2 * 20^2))
gaussian_vals = gaussian_func.(wave_range)

gaussian_func1(wave) = 70 * exp(-((wave - 470)^2) / (2 * 30^2))
gaussian_vals1 = gaussian_func1.(wave_range) 

gaussian_func2(wave) = 50 * exp(-((wave - 700)^2) / (2 * 40^2))
gaussian_vals2 = gaussian_func2.(wave_range)

# Plot spectral signatures

plot(wave_range, gaussian_vals, title="Spectral Signatures",
    xlabel="Wavelength", linewidth=2, left_margin=5Plots.mm, label="A(t)", ylabel="Δ Absorbance")
...
# CSV.write("/path/to/desired/location/2ndOrderData.csv", df)
