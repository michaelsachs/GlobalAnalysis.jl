...
In theory, we can calculare our kinetics to infinite time resolution, yet in practice, we encounter a limit to time resolution which is defiend by the instrument set up. Rather than having a sharp signal onset, the instrument response causes the onset to be smeared out. If we want to fit our experimental data to an appropriate model, we need to account for this response. Mathematically, this is done through convolution. In KineticModel.jl, we convolve the instrument response with a gaussian which has a defined position $μ$ (which may be slightly different from 0.0) and width $σ$. In that case of the Michaelis-Menten reaction system, our kinetic traces will be convolved.
...
Next, make a prediction for the parameter bounds. It is okay if the range is large, KineticModel.jl is able to handle large ranges well. 
...
```
Finally, you can run the fitting procedure.
```julia
# Run fitting procedure
OP = bboptimize(Objective; SearchRange = bounds)
```
This returns an optimized set of parameters for $u0 = [A, B]$, $p = [k_1, k_2]$, $\mu$, and $\sigma$. 